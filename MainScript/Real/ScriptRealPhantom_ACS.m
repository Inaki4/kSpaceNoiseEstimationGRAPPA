function [gFactorMC,gFactorK,gFactorX]=ScriptRealPhantom_ACS(kernel,R,SNR,PLOT,filepath)
%ScriptRealPhantom_ACS Function to generate the figure in the paper
...
% Std_Est_KSingleCoils=complex(sqrt(real(NoiseMapEstKCoils)),sqrt(imag(NoiseMapEstKCoils)));
...
%We save the results
save([filepath '/RealPhantom/Figure_ACS/ResultsACS_R' num2str(R) '_kernel' kernelstr '_SNR' num2str(SNR) '.mat'],'GammaK','CK','kernel','N','Walsh','Lines','mssLines','LnsKnl','mssKnl','Size','method','R','Reff','LinesACS','Std_MCAcc','Std_Est_KSingle','Std_Est_XSingle','Std_Est_Full_Single','Std_MCFull','gFactorMC','gFactorK','gFactorX')
end