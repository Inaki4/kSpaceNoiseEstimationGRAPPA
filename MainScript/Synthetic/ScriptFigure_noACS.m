function [gFactorMC,gFactorK,gFactorX]=ScriptFigure_noACS(kernel,R,SNR,rho,L,Size,NEX,PLOT,filepath)
%ScriptFigure_noACS Function to generate the figure in the paper
%comparing the g-factors in the case of uniform sampling
% Input parameters
...
W=chol(VTot)';
clear MAux MAux2 IndTriLow IndTriLowDiag IndTriUp
...
%We get the kernel and Walsh vector with the ACS lines and then reconstruct
%deleting them. First we corrupt the image with noise
Nc=randn([2*L,Size^2]);
Nc2=W*Nc;
Nc3=complex(Nc2(1:L,:),Nc2((L+1):(2*L),:));
Nc4=permute(reshape(Nc3,[L,Size,Size]),[2 3 1]);
kN=K+Nc4;
[Fc,~,N,LnsKnl,mssKnl]=recongrappa_multik([Size,Size,L],kN(Lines,:,:),Lines,'kernel',kernelstr); 
Sn=k2x(Fc,method);
[Ir,Walsh]=coilCombine_SingleSlice(Sn,[]);
...
for Exi=1:NEX
    ...
    k_SN(Lines,:,:)=kN(Lines,:,:); 
    Fc=Grappa_CircConv( k_SN,N,kernel,R,L);
    Sn=k2x(Fc,method);
    [Ir]=coilCombine_SingleSlice(Sn,Walsh);
...
%K-space estimation
%We make the reconstruction cyclic
...
%X-space estimation
[NoiseMapX,NoiseMapCrossX]=NoiseGRAPPA_XSpace(GammaK,CK,kernel,L,N,Walsh,Size,Lines,0);
...
if (PLOT)
    ...
    print(['/home/inaki/Escritorio/ProyectoMadison/CircularConv/Figure_noACS/NoiseMap_noACS_SNR' num2str(SNR) '_Size' num2str(Size)],'-dpng')
    ...
    print(['/home/inaki/Escritorio/ProyectoMadison/CircularConv/Figure_noACS/g-factor_noACS_SNR' num2str(SNR) '_Size' num2str(Size)],'-dpng')
    ...
    print(['/home/inaki/Escritorio/ProyectoMadison/CircularConv/Figure_noACS/NoiseMapCross_noACS_SNR' num2str(SNR) '_Size' num2str(Size)],'-dpng')
end

%We save the results
save([filepath '/Synthetic/Figure_noACS/ResultsnoACS_R' num2str(R) '_kernel' kernelstr '_SNR' num2str(SNR) '_Size' num2str(Size) '.mat'],'GammaK','CK','kernel','SNR','L','N','Walsh','Lines','mssLines','LnsKnl','mssKnl','Size','method','Reff','LinesACS','NoiseMapMCAcc','NoiseMapK','NoiseMapX','NoiseMapFull','NoiseMapMCFull','gFactorMC','gFactorK','gFactorX','NoiseMapCrossAcc','NoiseMapCrossK','NoiseMapCrossX')
end